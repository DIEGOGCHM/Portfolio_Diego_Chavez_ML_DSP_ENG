...
export default function About() {
  ...
  const handleSelect = (idx: number) => {
    if (containerRef.current) {
      ...
      // Calculate target progress. Add a tiny offset so it securely triggers the index change.
      const chunk = 1 / aboutData.length;
      // Target the middle of the chunk to avoid edge jitter
      const targetProgress = (idx * chunk) + (chunk / 2);
